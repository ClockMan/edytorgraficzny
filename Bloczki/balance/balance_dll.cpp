...
int __stdcall validate(Block *aBlock)
{
	if((aBlock->input.size() == 0) && (aBlock->output.size() == 0))
	{
		...
		aBlock->getConfig()->addInt("R",0);
		aBlock->getConfig()->addInt("G",0);
		aBlock->getConfig()->addInt("B",0);
...
int __stdcall run(Block *aBlock)
{
    ...
    int R(aBlock->getConfig()->getInt("R"));
    int G(aBlock->getConfig()->getInt("G"));
    int B(aBlock->getConfig()->getInt("B"));

	if(!RGBBalance(picture,R,G,B))
        {
	...
	TypeConfig* copy;       //typ taki sam jak na wejsciu:)
