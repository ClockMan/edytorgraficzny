...
int __stdcall run(Block *aBlock)
{
	...
	if(mode == 0)    //mediana
	{
        ...
        else if(mode == 1) //erozja
        {
		if(!StatisticsFilter(picture, 1))
		{
			aBlock->output[0].setErrorCode(2);
			aBlock->output[0].setErrorDescription("Pusta bitmapa");
			picture->Free();
        ...
        else if(mode == 2) //ekspansja
        {
		if(!StatisticsFilter(picture, 9))
		{
			aBlock->output[0].setErrorCode(2);
			aBlock->output[0].setErrorDescription("Pusta bitmapa");
			picture->Free();
